...
class ActorCritic(nn.Module):
    ...
    def take_action(self, state, id):
        # actions_mean = self.actor[id](state)
        if id == 1:
            actions_mean = self.actor1(state)
        else:
            actions_mean = self.actor2(state)
    ...
    def evaluate(self, state, action, id):
        # actions_mean = self.actor[id](state[:, :self.state_dim])
        if id == 1:
            actions_mean = self.actor1(state[:, :self.state_dim])
        else:
            actions_mean = self.actor2(state[:, :self.state_dim])
