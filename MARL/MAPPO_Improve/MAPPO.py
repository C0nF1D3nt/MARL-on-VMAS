...
def trainAgent():
    ...
    action_std = 0.5
    # 智能体
    agent = MAPPO(state_dim, action_dim, n_agents, actor_lr, critic_lr, gamma, epochs, eps, action_std)
    ...
    while step < max_step:
        ...
        for i in range(episode_len):
            ...
            action1, action2 = agent.select_action(state1, state2)
            ...
            agent.memory1.rewards.append(rewards1)
            agent.memory2.rewards.append(rewards2)
            agent.memory1.done.append(done[0])
            agent.memory2.done.append(done[0])
            # 累加平均奖励
