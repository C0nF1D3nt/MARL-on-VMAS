...
def trainAgent():
    ...
    action_std = 0.5
    # 智能体
    agent1 = IPPO(state_dim, action_dim, n_agents, actor_lr, critic_lr, gamma, epochs, eps, action_std)
    agent2 = IPPO(state_dim, action_dim, n_agents, actor_lr, critic_lr, gamma, epochs, eps, action_std)
    ...
    while step < max_step:
        ...
        for i in range(episode_len):
            ...
            action1 = torch.clamp(action1, -1.0, 1.0).unsqueeze(dim=0)
            action2 = torch.clamp(action2, -1.0, 1.0).unsqueeze(dim=0)
            ...
            agent1.memory.rewards.append(rewards1)
            agent2.memory.rewards.append(rewards2)
            agent1.memory.done.append(done[0])
            agent2.memory.done.append(done[0])
            # 累加平均奖励
            ...
            if step % learn_freq == 0:
                agent1.learn(n_agents)
                agent2.learn(n_agents)
            ...
            if step % std_change_freq == 0:
                agent1.change_action_std(action_std_change_rate, min_action_std)
                agent2.change_action_std(action_std_change_rate, min_action_std)
